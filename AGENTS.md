...
Instructions for AI coding assistants and developers working on this skills ecosystem. This file defines how to write, maintain, and use skills.

---

## Architecture

Skills are designed for **terminal-native execution** — every command runs via standard UNIX tools (curl, nmap, python3, subfinder, httpx, nuclei, ffuf). No special dependencies, no browser automation required.

**Skills are the knowledge layer.** Each SKILL.md is a complete operational package: when to use it, what commands to run, what to look for, how to verify results, and what pitfalls to avoid. Cross-references link skills together without duplication.

Two principles shape every skill:

- **Terminal-native:** Commands use standard tools available on any recon environment (Alpine/Debian Linux). curl, nmap, masscan, python3, and standard UNIX utilities.
- **Self-contained workflows:** A skill tells the agent everything it needs. Read one file, execute the workflow.
...
Always check `/root/output/recon_output/` first:
- `FINDINGS_REPORT.md` — overall status
...
- **All done:** Write report to `FINDINGS_REPORT.md`

### 5. Every Session Must Produce Output

Always save to `/root/output/recon_output/`. Every finding, every scan, every report.
...
### redteam/ — Per-Class Vulnerability Hunting 

**hunt-* skills :** One skill per vulnerability class. Each is built from public bug bounty reports and includes detection methodology, exploitation techniques, bypass tables, and real paid examples.
...
**recon-sector :** Unified parameterized sector recon with 25 sectors in `sectors.yaml` database. Replaces 25 individual `recon-*` skills.

**Methodology skills :** bb-methodology, bb-local-toolkit, bug-bounty, web2-recon, osint-methodology, offensive-osint.

**Operations skills :** parallel-recon-triad, ops-proxyns, evidence-hygiene, triage-validation, report-writing, redteam-report-template, bugcrowd-reporting, security-arsenal, redteam-mindset.

**Infrastructure skills :** cloud-iam-deep, enterprise-vpn-attack, m365-entra-attack, okta-attack, vmware-vcenter-attack, supply-chain-attack-recon, meme-coin-audit, mid-engagement-ir-detection, cors-chain-automation, wp-plugin-automation, wp-plugin-cve-hunt, wordpress-cors-xmlrpc-rce-chain, apk-redteam-pipeline, web3-audit.

### Environment 

- **controller**: Hermes Agent features guide — delegation, memory, YOLO mode, slash commands, Telegram setup.
- **worker**: Worker container environment — tools, paths, usage patterns, known pitfalls.

### Built-in Skills 

See Hermes Agent docs for the full list. Key categories:
- **Creative :** architecture-diagram, ascii-art, claude-design, comfyui, excalidraw, p5js, sketch
- **GitHub :** github-auth, github-pr-workflow, github-code-review, github-issues
- **Software Dev :** plan, spike, systematic-debugging, test-driven-development
- **MLOps :** huggingface-hub, llama-cpp, vllm, evaluating-llms-harness
...
- **sector recon**: `recon-sector` (parameterized, sectors in `sectors.yaml`)
...
```
/root/output/recon_output/
+-- FINDINGS_REPORT.md Consolidated master report
...
|-------|-----------|-------------|-------|
| XMLRPC status | 200 (79 methods) | REGRESSION |
