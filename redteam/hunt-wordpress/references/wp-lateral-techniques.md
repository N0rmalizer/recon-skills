...
**Field finding ([MATTRESS_RETAILER]):** Cart Token decoded to show temporary user ID (`t_*` prefix), 48h expiry, and `store-api` issuer. The token can be reused across requests to interact with cart/checkout endpoints.
...
**Key namespaces found in field ([MATTRESS_RETAILER], 23 total):**
...
page-sitemap.xml can be MASSIVE (43,981 URLs on [MATTRESS_RETAILER]). Don't skip it:
...
All 62 routes discovered from [MATTRESS_RETAILER]'s Yoast SEO Premium v27.8:
