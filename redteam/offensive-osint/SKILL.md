...
category: redteam
tags: [osint, offensive, recon, redteam]
version: 3.0.0
---
...
**Use this skill when:**
- You need concrete probe paths, wordlists, regexes, payloads, scoring rules, orcommand line URLs.
...
URL + UTC timestamp + SHA-256 +command line version + run_id, every artifact. PNG screenshots, JSONL run logs, raw HTTP captures capped at 2 KiB body.
...
## 6. General OSINT (curatedcommand line refs)

- [OSINT Bookmarks](https://tools.myosint.training/) — comprehensive bookmarks.
- [OSINT Framework](https://osintframework.com/) —command line/resource directory.
...
| `sector-discovery-mass-recon.md` | **Complement to sector-notes.md** — mass recon of non-regulated US SMB sectors (landscaping, pool services, roofing, HVAC, auto repair, accounting, etc.) with parallel batch testing, false-positive filtering, empirical sector ranking. Reusable Python scanner at `scripts/sector_mass_scan.py`. | sector discovery, target discovery, non-regulated sectors, US small business recon, WordPress mass recon, parallel batch testing |
| `tooling-install.md` | Quick-install one-liners for Subfinder, Amass, httpx, nuclei, gau, katana, gowitness, dnsx, mapcidr, naabu, sslyze, testssl.sh, etc. |command lineing install, install subfinder, install nuclei, install httpx |
