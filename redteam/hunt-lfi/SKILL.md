...
- **`hunt-rce`** — LFI is the most direct path to RCE on PHP/Java stacks through filter-chain, log poisoning, or phar:// deserialization. Chain primitive: php://filter iconv chain (Synacktiv, 2022) converts a file-read primitive to arbitrary PHP execution without any upload or writable file — just the include() call and a long filter URL → `?cmd=id` yields command output.
- **`hunt-source-leak`** — LFI file reads overlap with source code disclosure. Chain primitive: LFI reads `.env`, `wp-config.php`, `config.yml` → extracts DB credentials, API keys, secrets → feeds those into cloud-iam-deep for escalation.
- **`hunt-file-upload`** — Combined file-upload + LFI is the classic RCE chain. Chain primitive: upload an image with `GIF89a;<?php system($_GET['c']);?>` magic bytes → LFI includes `/uploads/exploit.jpg` → PHP now treats it as a script — RCE.
- **`hunt-ssrf`** — LFI via wrapper chains sometimes reaches internal network. Chain primitive: include a file that triggers SSRF or use RFI (`allow_url_include=On`) to fetch remote URLs — SSRF through the LFI primitive.
- **`hunt-source-leak`** — LFI reads source files that reveal other vulnerabilities. Chain primitive: LFI reads `index.php` → discovers include path and additional LFI parameters → expands attack surface.
- **`security-arsenal`** — Reach for the LFI wrapper tree (php://filter, php://input, data://, phar://, zip://, expect://) before testing manual payloads. The filter-chain RCE generator is in the arsenal references.
