...
```
controller (172.20.0.3)          worker (172.20.0.2) ← YOU ARE HERE
┌──────────────────────────┐          ┌──────────────────────────┐
│ /opt/data/ ← HERMES_HOME │  SSH    │ /root/                   │
│ ├── AGENTS.md       ✅   │────────→│ ├── output/recon_output/  ✅  │
│ ├── SOUL.md         ✅   │         │ ├──tools/            ✅  │
│ ├── skills/207      ✅   │         │ ├── scripts/          ✅  │
│ ├── config.yaml     ✅   │         │ ├── .ssh/             ✅  │
│ └── ssh/worker_key ✅  │         │ └── .hermes/skills/   ✅  │
│                          │         │                          │
│ Runs Hermes Agent       │         │ Runs recontools         │
...
## Worker Routing — Which worker to use

**Default worker** (`worker`) — use for ALL standard recon. You are already connected here.
...
```
SOCKS5 proxy at `worker-tor:9050`. Tor circuit may need VPS — ISP/Docker can block Tor network.
...
- **User**: `root`
- **Auth**: SSH key — the key lives at `/opt/data/ssh/worker_key` on the Hermes host, used to SSH into this worker
...
### ❌ NOT Installed
```
go          —        Go language (not available — pre-compiled Go binaries (e.g., pdtools) work, but compiling Go from source won't)
...
2. Set up a cron job with:
   ...
   - `skills: ["worker"]` for context (the skill does not exist — prompt must be self-contained)
...
- Tor worker: `torsocks curl --max-time 30 --connect-timeout 10 -s https://ifconfig.me` MUST return a different IP than the non-Tor worker.
...
- All curl commands MUST use `--max-time` and `--connect-timeout`.
