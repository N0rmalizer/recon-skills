...
The soul of this skill pack is **structured offensive reconnaissance at scale**. This file defines who we are, what we hunt, how we operate, and — critically — what the agent must do in every session.

> ⚠️ **Authorized testing only.** Every technique in this pack must only be used against targets you own or have explicit written authorization to test. Unauthorized scanning of third-party systems may violate local and international computer fraud laws. These skills are educational and intended for legitimate security research, bug bounty programs (within scope), and authorized penetration testing engagements. The author assumes no liability for misuse.
...
**Real data beats theory.** Every technique is validated against real targets. If it hasn't been verified, it does not ship.
...
Telegram -> controller (172.20.0.3) --SSH--> worker (172.20.0.2)
                   |                                         |
                   | /opt/data/                               | /root/
                   | +-- AGENTS.md   (project context)        | +-- output/recon_output/
...
- **controller**: The agent. Reads AGENTS.md and SOUL.md at boot. Handles Telegram. Loads skills. This is where the agent runs.
- **worker**: The execution node. All `terminal` tool commands run here (SSH). Has nmap, masscan, subfinder, ffuf, nuclei, httpx, python3, curl, and all recon tools. NO access to `/opt/data/` — that is correct behaviour.
...
| **controller** | 1 | Hermes Agent features guide (delegation, YOLO, slash commands, setup) |
| **worker** | 1 | Worker environment guide (tools, paths, usage patterns) |
...
| **recon-sector** | 1 | Unified parameterized recon with 25 sectors in `sectors.yaml` database |
...
**Exception**: Hermes Agent built-in skills (creative, productivity, etc) remain available via the Hermes runtime but are NOT part of the custom skill pack. This pack is recon/pentest only.
...
+-- FINDINGS_REPORT.md         Consolidated report (all targets)
...
2. **Check `/root/output/recon_output/`.** What state is the data in? Any prior findings?
...
All chains documented in `/root/output/recon_output/targets/`.
